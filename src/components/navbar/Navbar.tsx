...
const Navbar = ({setType, setContext}: Props) => {
  ...
  return (
    <nav className='flex items-center justify-around w-full min-h-[60px] bg-[#404040]' >
      ...
      {
        type == "2" ? 
        <button onClick={() => setDisplayDebug(true)} className='p-[20px]'
        >
...

      {
        ...
        >
            ...
            <div className="flex flex-row justify-evenly mt-[30px] items-center">
                <h1>Input the URL :</h1>
                <input type='text' name="context" className='rounded-[5px] text-[20px] text-[black] w-[300px]' onChange={(e:ChangeEvent<HTMLInputElement>) => {onInputchange(e)}}/>
            </div>
            <div className='flex justify-evenly mt-[30px]'>
              ...
              <button className="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center" onClick={() => setDisplayDebug(false)}>
                Cancel
              </button>
